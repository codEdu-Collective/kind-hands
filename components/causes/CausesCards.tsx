...
const CausesCards = (): JSX.Element => {
  return (
    <section className="w-full py-16 bg-[#F7F5F2]">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {causesData.map((cause) => (
            <div key={cause.id} className="flex flex-col bg-white shadow-sm group">
              <div className="relative h-[300px] w-full overflow-hidden">
                ...
                <div className="absolute top-6 left-6 bg-white px-3 py-2 min-w-[50px] text-center">
                  <span className="block text-rose-500 font-bold text-sm">{cause.percentage}%</span>
                </div>
              </div>

              <div className="p-8 pb-10">
                <Link href={`/causes/${cause.id}`} className="cursor-pointer">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="bg-gray-100 rounded-full px-6 py-3 w-full sm:w-auto text-center">
                    <span className="text-gray-600 text-xs font-semibold tracking-wide">
                      Donation : ${cause.raised} / ${cause.goal}
                    </span>
                  </div>
