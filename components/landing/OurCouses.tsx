...
const OurCouses = async () => {
  ...
  return (
    <section className="w-full min-h-332 bg-beige flex flex-col items-center justify-center text-dark py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-9 mt-8 sm:mt-12 w-full max-w-300 px-4 sm:px-9">
        {causes.map(
          ({ id, percentage, raised, goal, imageUrl, title }, index) => (
            <div key={index} className="relative w-full">
              ...
              <div className="bg-background dark:bg-gray-800 w-full p-4 sm:p-6 md:p-8">
                <h2 className="font-shippori text-lg sm:text-xl md:text-2xl font-extrabold mb-4 sm:mb-6 md:mb-8 dark:text-white">
                  {title}
                </h2>
                ...
                <button className="uppercase w-full sm:w-36 h-12 sm:h-14 md:h-16 bg-dark dark:bg-primary rounded-4xl text-white text-sm sm:text-base md:text-lg font-work font-medium">
                  Donate
                </button>
              </div>
            </div>
